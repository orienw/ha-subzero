...
Control changes are confirmed from appliance status, not from the command acknowledgement. Property writes use up to three attempts, each with an eight-second deadline covering the request and confirmation. A command error or missing push confirmation prompts a status read; authentication and rate-limit errors stop immediately. Each retry rechecks whether the change is still allowed. Ice modes and remote starts preserve the app's ordered writes, including repeated values, and later writes stop if a setting cannot be confirmed. Kitchen-timer restarts require a fresh update or status read. Failed confirmation includes the last command error when one was reported.
